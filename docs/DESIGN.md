...
- **Versioning:** `CHANGELOG.md` is the single source of truth. Add a new
  `## [X.Y.Z]` entry at the top, then run `python3 tool/sync_version.py` — it
  propagates that version into `kAppVersion` (`lib/app_info.dart`) and
  `pubspec.yaml` (build number = number of changelog entries). The build runs
  this automatically, and `test/version_sync_test.dart` fails if they drift, so
  the displayed version always matches the changelog.
- **Icons:** generated by `tool/make_icons.py` from `tool/icon_source.png`
  (a 1024×1024 wooden-tiles artwork with the offline badge and wordmark);
  standard icons use the full image, maskable icons scale it into the Android
  safe zone on a white field.
